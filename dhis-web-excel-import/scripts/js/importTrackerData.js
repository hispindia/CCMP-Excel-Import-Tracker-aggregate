...
function importExcelData(trackedEntityInstanceObj, enrollmentObj, eventObj, row){
    ...
    function handleRegistrationSuccess(data)
    {
        ...
        if(data.response.conflicts != undefined)
        {
            var errorValue =  data.response.conflicts[0].value;
    ...
    {
        if(textStatus.status == 409 && textStatus.statusText == "Conflict" )
        {
            //var responseData = JSON.stringify(textStatus.responseText);
            var responseData = JSON.parse(textStatus.responseText)
            console.log( JSON.parse(textStatus.responseText) );
            //console.log( textStatus.responseText.response.conflicts[1] );

            var errorValue =  responseData.response.conflicts[0].value;
            var Uid = errorValue.split(" ").pop();
            ...
            if(Uid === 'qMFxuARyJHj')
            {
                //errorValue = errorValue.replace(Uid, '<b>Patient Id</b>');
                //$('#summary > tbody:last').append('<tr class="alert-info" ><td>Failed</td><td>1</td><td>'+errorValue+'</td></tr>');
                $('#summary > tbody:last').append('<tr class="alert-info" ><td>Failed</td><td>1</td><td>Registration attributes are found duplicate for row: <b>'+(row + 1)+' </b></td></tr>');
            }
            else
            {
                $('#summary > tbody:last').append('<tr class="alert-info" ><td>Failed</td><td>1</td><td>Registration attributes are found duplicate for row: <b>'+(row + 1)+' </b></td></tr>');
